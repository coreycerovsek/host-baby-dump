...
(function($, undefined) {
		...
		if ( isMethodCall ) {
			...
			this.each(function() {
				...
				if ( instance ) {
					...
					$.data( this, name, new $.jPlayer( options, this ) ); // eslint-disable-line
				}
			});
		}
	...
	$.jPlayer.uaBrowser = function( userAgent ) {
		...
		// Useragent RegExp
		var rwebkit = /(webkit)[ \/]([\w.]+)/; // eslint-disable-line no-useless-escape
		var ropera = /(opera)(?:.*version)?[ \/]([\w.]+)/; // eslint-disable-line no-useless-escape
		var rmsie = /(msie) ([\w.]+)/;
	...
	if ( platformMatch.platform ) {
		...
		$.jPlayer.platform.tablet = Boolean(platformMatch.tablet);
	}
	...
	$.jPlayer.prototype = {
		...
		_init: function() {
			...
			this.internal.flash = $.extend({}, {
				...
				jq: undefined,
				swf: this.options.swfPath + (this.options.swfPath.toLowerCase().slice(-4) !== ".swf" ? (this.options.swfPath && this.options.swfPath.slice(-1) !== "/" ? "/" : "") + "Jplayer.swf" : "") // eslint-disable-line no-negated-condition
			});
			...
			if(this.require.audio) { // If a supplied format is audio
				...
				this.html.audio.available = Boolean(this.htmlElement.audio.canPlayType) && this._testCanPlayType(this.htmlElement.audio); // Test is for IE9 on Win Server 2008.
			}
			this.html.video.available = false;
			if(this.require.video) { // If a supplied format is video
				this.htmlElement.video = document.createElement("video");
				this.htmlElement.video.id = this.internal.video.id;
				this.html.video.available = Boolean(this.htmlElement.video.canPlayType) && this._testCanPlayType(this.htmlElement.video); // Test is for IE9 on Win Server 2008.
			}
		...
		_addHtmlEventListeners: function(mediaElement, entity) {
			...
			mediaElement.addEventListener("play", function() {
				if(entity.gate) {
					...
					self._htmlCheckWaitForPlay(); // So the native controls update this variable and puts the hidden interface in the correct state. Affects toggling native controls.
		...
		_trigger: function(eventType, error, warning) { // eventType always valid as called using $.jPlayer.event.eventType
			var event = $.Event(eventType); // eslint-disable-line
			event.jPlayer = {};
				...
				switch(eventType) {
					...
					case $.jPlayer.event.error:
						...
						if(this.status.video) { // Set up for another try. Execute before error event.
							this._flashSetVideo(this.status.media);
						} else {
							this._flashSetAudio(this.status.media);
						}
						this._updateButtons(false);
						this._error( {
							type: $.jPlayer.error.URL,
					...
					if(self[solution].support[format] && self._validString(media[format])) { // Format supported in solution and url given for format.
						...
						if(isVideo) {
							if(isHtml) {
								...
								self._htmlSetVideo(media);
								self.html.active = true;
							} else {
								self.flash.gate = true;
								self._flashSetVideo(media);
							...
							if(isHtml) {
								...
								self._htmlSetAudio(media);
								self.html.active = true;
							} else {
								self.flash.gate = true;
								self._flashSetAudio(media);
								self.flash.active = true;
							}
							if(self.css.jq.videoPlay.length) {
		...
		_muted: function(muted) {
			...
			if(this.html.used) {
				this._htmlMute(muted);
			}
			if(this.flash.used) {
				this._flashMute(muted);
			}
		...
		mute: function(mute) { // mute is either: undefined (true), an event object (true) or a boolean (muted).
			mute = mute === undefined ? true : Boolean(mute);
			this._muted(mute);
		},
		unmute: function(unmute) { // unmute is either: undefined (true), an event object (true) or a boolean (!muted).
			unmute = unmute === undefined ? true : Boolean(unmute);
			this._muted(!unmute);
		},
		...
		_htmlPlayHead: function(percent) {
			var self = this;
			this._htmlLoad(); // Loads if required and clears any delayed commands.
			try {
		...
		_htmlCheckWaitForPlay: function() {
		...
		_flashResetMedia: function() {
			this.internal.flash.jq.css({"width":"0px", "height":"0px"}); // Must do via CSS as setting attr() to zero causes a jQuery error in IE.
			this._flashPause(NaN);
		},
		_flashClearMedia: function() {
			try {
		...
		_flashPlay: function(time) {
			...
			this.status.waitForLoad = false;
			this._flashCheckWaitForPlay();
		},
		_flashPause: function(time) {
			try {
		...
		_flashCheckWaitForPlay: function() {
			if(this.status.waitForPlay) {
				this.status.waitForPlay = false;
				if(this.css.jq.videoPlay.length) {
					this.css.jq.videoPlay.hide();
		...
		_alert: function(message) {
			alert("jPlayer " + this.version.script + " : id='" + this.internal.self.id + "' : " + message); //eslint-disable-line no-alert
		},
